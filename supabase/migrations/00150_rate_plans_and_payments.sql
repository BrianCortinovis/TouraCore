-- Migration 00150: Payment rate plans + payment methods/attempts + reservation payment state
-- Fasi 2-6 del refactor billing Stripe Connect Direct Charge.
--
-- NOTA: tabella si chiama 'payment_rate_plans' per non collidere con la legacy
-- 'rate_plans' del modulo hospitality (rate codes/meal plan).
--
-- Payment rate plans: tariffe selezionabili per entity (4 tipi) con cancellation policy
...
-- Colonne su reservations: payment_rate_plan_id, charge_scheduled_at, payment_state

-- ============================================================
-- 1. PAYMENT RATE PLANS (per entity, tutti verticali)
-- ============================================================

CREATE TABLE IF NOT EXISTS public.payment_rate_plans (
  ...
  refund_window_hours      INT NOT NULL DEFAULT 168,
...
CREATE INDEX IF NOT EXISTS idx_payment_rate_plans_entity
  ON public.payment_rate_plans (entity_id, active, sort_order);

CREATE UNIQUE INDEX IF NOT EXISTS uq_payment_rate_plans_default_per_entity
  ON public.payment_rate_plans (entity_id) WHERE is_default = true;

ALTER TABLE public.payment_rate_plans ENABLE ROW LEVEL SECURITY;

CREATE POLICY payment_rate_plans_tenant_read ON public.payment_rate_plans
...
CREATE POLICY payment_rate_plans_tenant_write ON public.payment_rate_plans
  FOR ALL TO authenticated
  USING (tenant_id = ANY(public.get_user_tenant_ids()))
  WITH CHECK (tenant_id = ANY(public.get_user_tenant_ids()));

CREATE POLICY payment_rate_plans_public_read ON public.payment_rate_plans
  FOR SELECT TO anon
  USING (active = true);

CREATE POLICY payment_rate_plans_service_all ON public.payment_rate_plans
  FOR ALL TO service_role USING (true) WITH CHECK (true);

COMMENT ON TABLE public.payment_rate_plans IS
  'Tariffe pagamento selezionabili per entity. 4 tipi: free_cancellation, deposit_30, partially_refundable_50, non_refundable. Vale per tutti 4 verticali. Distinta da legacy rate_plans (hospitality rate codes).';
...
ALTER TABLE public.reservations
  ADD COLUMN IF NOT EXISTS payment_rate_plan_id UUID REFERENCES public.payment_rate_plans(id) ON DELETE SET NULL,
...
DROP TRIGGER IF EXISTS payment_rate_plans_set_updated ON public.payment_rate_plans;
CREATE TRIGGER payment_rate_plans_set_updated
  BEFORE UPDATE ON public.payment_rate_plans

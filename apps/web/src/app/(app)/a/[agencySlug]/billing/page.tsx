...
export default async function BillingPage({ params }: BillingPageProps) {
  ...
  return (
    <div className="space-y-6 px-6 py-6">
      <header>
        <h1 className="text-2xl font-semibold">Billing · {agency.name}</h1>
        <p className="mt-1 text-sm text-slate-600">
          Subscription piattaforma, fatture ricevute, Stripe Connect per payout.
        </p>
      </header>
      ...
      <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          <p className="mt-1 text-lg font-semibold capitalize">{agency.plan.replace('agency_', '')}</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <p className="text-xs uppercase text-slate-400">Stripe Connect</p>
          <p className="mt-1 text-sm">
            {agency.stripe_connect_account_id
              ? <span className="text-emerald-700">Attivo · {agency.stripe_connect_account_id.slice(0, 10)}…</span>
              : <span className="text-amber-700">Non configurato</span>}
          </p>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        {!canWrite ? (
          <p className="mt-2 text-sm text-slate-500">Solo admin/owner possono modificare il piano.</p>
        ) : (
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <p className="mt-2 text-sm text-slate-500">
          {agency.stripe_subscription_id
            ? `Subscription ${agency.stripe_subscription_id.slice(0, 14)}… · dettagli in Stripe dashboard`
            : 'Subscription non attiva. Configura Stripe per ricevere fatture.'}
        </p>
      </section>
    </div>
  )

...
export function LinkClientForm({ agencySlug }: { agencySlug: string }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      <h3 className="text-sm font-semibold">Collega cliente già registrato</h3>
      <p className="mt-1 text-xs text-slate-500">
        Inserisci l&apos;identificativo URL del cliente (creato autonomamente o dall&apos;amministratore).
      </p>
      <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-4">
        <label className="text-sm md:col-span-2">
          <span className="text-slate-600">Identificativo cliente (URL)</span>
          <input
